...
describe('automobile tab', () => {
  beforeEach(()=>{
    cy.visit('http://sampleapp.tricentis.com/101/index.php');
    cy.get('#nav_automobile').click();
  })

  it('should display automobile form', () => {
    cy.get('#selectedinsurance').contains('Automobile');
  });
  ...
  it('should fill up all fields in product form', () => {
    ...
    cy.fillProductData('auto');

    cy.get('#startdate').should('have.value','03/31/2025');
    cy.get('#insurancesum').should('have.value',15000000);
    cy.get('#meritrating').should('have.value','Bonus 6');
    cy.get('#courtesycar').should('have.value','Yes');
